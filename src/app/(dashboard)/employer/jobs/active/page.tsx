...
export default async function EmployerActiveJobsPage() {
  ...
  const jobs = await getEmployerJobs(employerCompany.companyId, "active");

  return (
    <Section aria-labelledby="active-jobs-heading">
      <div className="mb-8 flex flex-col gap-2">
        <h1 id="active-jobs-heading" className="text-2xl font-semibold text-foreground sm:text-3xl">
          Active jobs
        </h1>
        <p className="text-muted-foreground" aria-live="polite">
          {jobs.length} {jobs.length === 1 ? "job" : "jobs"} currently live on Jobnura
        </p>
      </div>
      ...
      <Card padding="lg">
        <EmployerJobList
          ...
          emptyDescription="Jobs that pass review and go live will appear here."
        />
      </Card>
    </Section>
  );
