...
import { EMPLOYER_NAV_ITEMS } from "@/constants/employerNavigation";

/**
 * Persistent secondary navigation for the whole /employer area — mirrors
 * CandidateNav.tsx's own active-highlight pattern exactly (same
...
export function EmployerNav() {
  ...
  return (
    <nav aria-label="Employer portal" className="border-b border-border bg-surface">
      <div className="mx-auto flex max-w-6xl gap-6 overflow-x-auto px-4 sm:px-6 lg:px-8">
        {EMPLOYER_NAV_ITEMS.map((item) => {
          const isActive = item.href === "/employer" ? pathname === item.href : pathname?.startsWith(item.href);
          return (
            <Link
              key={item.href}
              href={item.href}
